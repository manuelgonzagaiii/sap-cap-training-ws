// Grader for Lesson 9 / Stage 2. A bound action with a parameter and return type, plus an
// unbound service function, both declared and implemented.
const fs = require("fs");
...
describe("Lesson 9 / s02 - parameters and unbound functions", () => {
  ...
  test("both operations are implemented in the handler", () => {
    expect(/\.on\s*\(\s*['"]adjustStock['"]/.test(impl)).toBe(true);
    expect(/\.on\s*\(\s*['"]lowStockCount['"]/.test(impl)).toBe(true);
  });
});
